...
contract Account{
	...
	function Account(bytes32 _companyName) public {
		...
		address loyaltyPointAddress = new LoyaltyPoint(companyName, 0, 100);
	...
	function addLoyaltyPoint(bytes32 _companyName, int points, uint rate) public returns(address) {
		LoyaltyPoint newLoyaltyPointAddress=new LoyaltyPoint(_companyName,points,rate);
		loyaltyPoints[_companyName]=newLoyaltyPointAddress;
