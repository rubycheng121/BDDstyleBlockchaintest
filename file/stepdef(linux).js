...
defineSupportCode(function ({ Given, When, Then, And, Before, After }) {
    ...
    Given('original blp account of A is {int}', function (int, callback) {
        ...
        //增加第一家公司紀錄的第二家公司的點數
        AccountA_contract.methods.addLoyaltyPoint('0x01', int, (rateB / rateA) * 100).send({
            from: web3.eth.defaultAccount,
            gas: 44444444
        }).then(function(){callback()})
    });
    ...
    Given('original alp account of B is {int}', function (int, callback) {
        ...
        //增加第二家公司紀錄的第一家公司的點數
        AccountB_contract.methods.addLoyaltyPoint('0x00', int, (rateA / rateB) * 100).send({
            from: web3.eth.defaultAccount,
            gas: 44444444
        }).then(function(){callback()})
    });
    ...
    When('A want to exchange {int} alp for blp', function (int, callback) {
        ...
        Exchange_contract.deploy({
            ...
            //兩家公司交換點數
            Exchange_contract.methods.to('0x00', '0x01', int).send({
                from: web3.eth.defaultAccount,
                gas: 44444444
            }).then(function(){callback()})
        })
    ...
    Then('alp account of A should be {int}', function (int, callback) {
        ...
        LoyaltyPointAA_contract.methods.getPoints().call().then((result)=>{
            if (result == int) {
                ...
                throw "LoyaltyPointAA_contract.getPoints() = " + result;
            }
        })
    });
        ...
        AccountA_contract.methods.getLoyaltyPoint('0x01').call().then((address)=>{
            ...
            LoyaltyPointAB_contract.methods.getPoints().call().then((result)=>{
                if (result == int) {
                    ...
                    throw "LoyaltyPointAB_contract.getPoints() = " + result;
                }
            })
        })
        ...
        AccountB_contract.methods.getLoyaltyPoint('0x00').call().then((address)=>{
            ...
            LoyaltyPointBA_contract.methods.getPoints().call().then((result)=>{
                if (result == int) {
                    ...
                    throw "LoyaltyPointBA_contract.getPoints() = " + result;
                }
            })
        })
    });
    ...
    Then('blp account of B should be {int}', function (int, callback) {
        ...
        LoyaltyPointBB_contract.methods.getPoints().call().then((result)=>{
            if (result == int) {
                ...
                throw "LoyaltyPointBB_contract.getPoints() = " + result;
            }
        })
    });
